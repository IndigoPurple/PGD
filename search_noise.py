...
print('*****************Add noise on std')
dataset_list = ['karate','dolphins','jazz','netscience','cora_ml', 'power_grid']
for dataset in dataset_list:
    ...
    model = torch.load("./model/i-deepis_"+dataset+".pt")
    ###NoisyTune
    for noi in range(0, 301, 20):
        noise = noi/1000
        print('#')
        print('noise = ', noise)
        noise_lambda = torch.tensor(noise).cuda()
        for name ,para in model.named_parameters():
            # model.state_dict()[name][:] += 0
            if name == 'gnn_model.fcs.2.bias':
                continue
            # print(name)
            # print(model.state_dict()[name][:].data.mean(), model.state_dict()[name][:].data.std())
            model.state_dict()[name][:] += torch.rand(para.size()).cuda() * noise_lambda * torch.std(para)
            # model.state_dict()[name][:] += torch.rand(para.size()).cuda() * noise_lambda * para
        # exit()
        # noise_level = torch.tensor(5/100).cuda()
        # w = model.weight
        # w = w.mul(1 + noise_level * torch.rand_like(w))
        # model.weight = w
        # exit()
        ...
        #     # model.state_dict()[name][:] += 0
        #     print(name)
        #     print(net.state_dict()[name][:].data.mean(), net.state_dict()[name][:].data.std())
        #     net.state_dict()[name][:] += torch.rand(para.size()) * noise_lambda * torch.std(para)
        net.train()
        ...
        if noi ==0:
            orig_test_acc = test_acc / (len(influ_mat_list) - num_training)
            print('original test acc', orig_test_acc)
        else:
            print('diff to orig acc:', test_acc / (len(influ_mat_list) - num_training) - orig_test_acc)

print('*****************Add noise on magnitude')
dataset_list = ['karate', 'dolphins', 'jazz', 'netscience', 'cora_ml', 'power_grid']
for dataset in dataset_list:
    print('--------------------')
    print(dataset)
